...
def get_red_mask(frame: FrameDictionary) -> np.ndarray:
    """Return a binary mask of red pixels in the entire frame."""
    mask1 = cv2.inRange(frame.hsv, np.array(_hsv_bounds['lo1']), np.array(_hsv_bounds['hi1']))
    mask2 = cv2.inRange(frame.hsv, np.array(_hsv_bounds['lo2']), np.array(_hsv_bounds['hi2']))
    return mask1 | mask2

def is_in_front(frame: FrameDictionary) -> bool:
...
def has_passed_red_line(frame: FrameDictionary) -> bool:
